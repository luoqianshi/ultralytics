...
def carafe(x, normed_mask, kernel_size, group=1, up=1):
    ...
    Args:
        x: [N, C, H, W] 输入特征。
        normed_mask: [N, kernel_size**2 * group, up*H, up*W] 已归一化的内容感知重组核。
    """
...
class FreqFusion(nn.Module):
    def __init__(self,
        ...
        assert hr_channels == lr_channels, 'FreqFusion 要求 HR/LR 特征通道数一致（forward 末尾逐元素相加）'
        ...
        if self.feature_resample:
            self.dysampler = LocalSimGuidedSampler(in_channels=self.compressed_channels, scale=2, style='lp', groups=feature_resample_group, use_direct_scale=True, kernel_size=encoder_kernel, norm=feature_resample_norm)
