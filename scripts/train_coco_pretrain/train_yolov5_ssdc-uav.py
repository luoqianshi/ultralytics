...
def train():
    """
    YOLOv11 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv11 模型，并进行公平性配置以对比其他框架（如 MMDetection）。
    """
    # 1. 配置路径
    ...
    # 这里以 yolov5su 为例，它是一个中等大小的模型，在 GPU 上训练速度和内存占用都适中
    model = YOLO('yolov5su.pt') 
    ## @TODO:重零训练模型的配置文件在：ultralytics\cfg\models\5
    # model = YOLO(r'ultralytics/cfg/models/5/yolov5su.yaml')
    ...
    results = model.train(
        ...
        name='yolov5su_ssdc-uav_exp01',# 实验名称
