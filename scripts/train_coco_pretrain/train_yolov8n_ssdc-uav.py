...
def train():
    """
    YOLOv8n 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv8n 模型，并进行公平性配置以对比其他框架（如 MMDetection）。
    """
    # 1. 配置路径
    ...
    # 这里以 yolov8n 为例，它是一个中等大小的模型，在 GPU 上训练速度和内存占用都适中
    model = YOLO('yolov8n.pt') 
    ## @TODO:重零训练模型的配置文件在：ultralytics\cfg\models\8
    # model = YOLO(r'ultralytics/cfg/models/8/yolov8n.yaml')
    ...
    results = model.train(
        ...
        name='yolov8n_ssdc-uav_exp01',# 实验名称
