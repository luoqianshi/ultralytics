...
def train():
    """
    YOLOv12 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv12 模型，并进行公平性配置以对比其他框架（如 MMDetection）。
    """
    # 1. 配置路径
    # 指定用户提供的 dataset.yaml 配置文件路径
    # 请确保此路径指向正确的数据集配置文件
    # 注意：根据用户之前的输入，路径可能在 cfg/datasets 下，也可能在 datasets/LAFSSDC_yolo 下
    ...
    # 2. 加载模型
    # 选项: yolo12n.pt, yolo12s.pt, yolo12m.pt, yolo12l.pt, yolo12x.pt
    # 'n' (nano) 是最快最小的模型，'x' (extra large) 是最准确但也最慢的模型
    # 这里以 yolo12s 为例，它是一个中等大小的模型，在 GPU 上训练速度和内存占用都适中
    ...
    results = model.train(
        ...
        name='yolo12s_ssdc-uav_exp01',# 实验名称
