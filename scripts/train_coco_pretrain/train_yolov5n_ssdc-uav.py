...
def train():
    """
    YOLOv5n 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv5n 模型，并进行公平性配置以对比其他框架（如 MMDetection）。
    """
    # 1. 配置路径
    # 指定用户提供的 dataset.yaml 配置文件路径
    # 请确保此路径指向正确的数据集配置文件
    # 注意：根据用户之前的输入，路径可能在 cfg/datasets 下，也可能在 datasets/SSDC-UAV_yolo 下
    # 这里我们优先使用用户最新指定的路径，如果不存在则回退
    ...
    # 2. 加载模型
    # 'n' (nano) 是最快最小的模型，'x' (extra large) 是最准确但也最慢的模型
    # 这里以 yolov5nu 为例，它是一个中等大小的模型，在 GPU 上训练速度和内存占用都适中
    model = YOLO('yolov5nu.pt') 
    ## @TODO:重零训练模型的配置文件在：ultralytics\cfg\models\5
    # model = YOLO(r'ultralytics/cfg/models/5/yolov5nu.yaml')
    ...
    # - Epochs (训练轮数): 
    #   MMDetection 的 DETR 配置文件 (detr_r50_lafssdc.py) 设置为 150 epochs。
    #   为了保持“相同的训练epoch数”并兼顾 YOLO 收敛需求，这里统一设置为 150。
    # - Optimizer (优化器):
    #   DETR 使用 AdamW (lr=2.5e-5, weight_decay=0.0001)。
    #   YOLO 默认使用 SGD。
    # - Input Size (输入尺寸):
    #   DETR 配置文件中使用 Resize(scale=(640, 640), keep_ratio=True)。
    #   YOLO 设置 imgsz=640 即可对齐。
    # - Data Augmentation (数据增强):
    #   DETR 仅使用了 Resize 和 RandomFlip。
    ...
    results = model.train(
        ...
        epochs=200,               # [对齐] 与 DETR 实验保持一致
        imgsz=640,                # [对齐] 输入图像尺寸
        batch=16,                 # 批次大小 (显存允许的情况下尽量大，16是官方推荐)
        project='runs/ssdc-uav_train',   # 训练结果保存的项目目录
        name='yolov5nu_ssdc-uav_exp01',# 实验名称
