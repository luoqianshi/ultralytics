...
def train():
    """
    YOLOv12 + Focaler-CIoU 训练脚本
    - 预训练权重: yolo12s.pt (COCO)
    - 数据集: SSDC-UAV
    - 训练: 150 epochs, imgsz=640, batch=16, SGD
    - 损失:Focaler-CIoU 赋能 CIoU (focaler_ciou=True)
    ...
    ## @TODO:重零训练模型的配置文件在：ultralytics\scripts\improved_yolo12\yolo12s.yaml
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12s.yaml')
    ...
    results = model.train(
        ...
        name='yolo12s_Focaler-CIoU_ssdc_uav_exp1_re0',  # 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        # Focaler-IoU 配置 (新增)
        focaler_ciou=True,        # 启用 Focaler-CIoU 赋能 CIoU
    )
