...
def train():
    """
    YOLOv12-DySample-plus 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv12-DySample-plus 模型，并进行公平性配置。
    """
    # 1. 配置路径
    ...
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12s-DySample-plus.yaml')
    ...
    results = model.train(
        ...
        name='yolo12s_SD-Loss_DySample-plus_ssdc_uav_exp01',# 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        pretrained='yolo12s.pt',  # 加载预训练权重
        # SD Loss 配置 (新增)
        sd_loss=True,            # 启用 SD Loss
        # 确定性配置 (新增)
        deterministic=False,      # 去除确定性训练
    )
