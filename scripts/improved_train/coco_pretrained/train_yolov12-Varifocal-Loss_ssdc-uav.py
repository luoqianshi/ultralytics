...
def train():
    """
    YOLOv12 + Varifocal Loss 训练脚本
    - 预训练权重: yolo12s.pt (COCO)
    - 数据集: SSDC-UAV
    - 训练: 150 epochs, imgsz=640, batch=16, SGD
    - 损失: Varifocal Loss (varifocal_loss=True), 分类分支以 IACS 软标签训练
    - 架构: 完全不变 (Backbone/Head/DFL/BBox损失 全部保留, 仅替换分类损失)
    """
    # 1. 配置路径
    ...
    results = model.train(
        ...
        epochs=150,               # 与 baseline 对齐 (E0': 150ep)
        imgsz=640,                # [对齐] 输入图像尺寸
        batch=16,                 # 批次大小 (显存允许的情况下尽量大，16是官方推荐)
        project='runs/ssdc_uav_train',   # 训练结果保存的项目目录
        name='yolo12s_Varifocal_Loss_ssdc_uav_exp1',  # 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        # Varifocal Loss 配置 (新增)
        varifocal_loss=True,    # 启用 Varifocal Loss (分类分支替换 BCE, IACS 软标签)
        varifocal_alpha=0.75,   # 负样本降权平衡因子 (官方默认)
        varifocal_gamma=2.0,    # 调制因子聚焦参数 (官方默认)
    )
