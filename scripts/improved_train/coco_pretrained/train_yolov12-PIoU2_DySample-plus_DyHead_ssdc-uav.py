...
def train():
    """
`   YOLO12 + DySample + DyHead + Powerful-CIoU2 训练脚本
    - 预训练权重: yolo12s.pt (COCO)
    - 数据集: SSDC-UAV
    - 训练: 150 epochs, imgsz=640, batch=16, SGD
    - 损失: Powerful-CIoU2 训练脚本
    """
    # 1. 配置路径
    ...
    # 2. 加载模型
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12s-DySample-plus_DyHead.yaml')
    ...
    results = model.train(
        ...
        name='yolo12s_PIoU2-DySample-plus_DyHead_ssdc_uav_exp1',  # 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        pretrained='yolo12s.pt',  # 加载 COCO 预训练权重
        # Powerful-CIoU2 配置 (新增)
        powerful_iou=True,        # 启用 Powerful-CIoU2
        # 关闭确定性训练
        deterministic=False,
    )
