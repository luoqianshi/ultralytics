...
def train():
    """
    YOLOv12-FreqFusion 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv12-FreqFusion 模型，并进行公平性配置。
    """
    # 1. 配置路径
    ...
    # 2. 加载模型
    # 说明：head 经过重构（1x1 横向压缩 + FreqFusion 替代 Upsample+Concat），
    # 仅 backbone（layer 0-8）可从 yolo12s.pt 迁移，head 全新初始化属预期行为，
    # 迁移率低于 DySample 等保留 head 主体的方案。
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12s-FreqFusion.yaml')
