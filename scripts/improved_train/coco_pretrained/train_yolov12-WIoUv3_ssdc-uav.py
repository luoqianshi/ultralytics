...
def train():
    ...
    # 2. 加载模型
    # WIoU v3 不修改模型架构，直接加载官方 COCO 预训练 YOLO12s
    # (与 baseline train_yolov12_ssdc-uav_300Epoch.py 一致)
    ...
    # 3. 开始训练
    # 与 baseline 完全对齐：epochs=300, imgsz=640, batch=16, SGD
    # 唯一差异：启用 WIoU v3 (wiou=True, wiou_alpha=1.0, wiou_momentum=0.01)
    # wiou_alpha=1.0 表示纯 WIoU v3 (不与 CIoU 混合)
    # wiou_momentum=0.01 为官方默认 EMA 动量 (慢响应，半衰期 ~69 batch)
    #
    # 监控点 (per spec §4.2):
    # - 第 30 epoch: loss_iou 应正常下降 (若 >2× baseline，r 压制过度)
    # - 第 50 epoch: iou_mean buffer 应从 1.0 降到 ~0.5
    # - 第 100 epoch: mAP_small 是否超过 baseline 的 0.3366
    ...
    results = model.train(
        ...
        name='yolo12s_WIoUv3_ssdc_uav_exp1',  # 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        # WIoU v3 配置 (新增)
        wiou=True,                # 启用 Wise-IoU v3 替代 CIoU
        wiou_alpha=1.0,           # 纯 WIoU v3 (1.0=纯, 0.5=与 CIoU 各半)
        wiou_momentum=0.01,       # iou_mean EMA 动量 (官方默认, 慢响应)
    )
