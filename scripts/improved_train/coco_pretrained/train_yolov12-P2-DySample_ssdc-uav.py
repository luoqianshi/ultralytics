...
def train():
    """
    YOLOv12-P2-DySample 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv12-P2-DySample 模型，并进行公平性配置。
    """
    # 1. 配置路径
    ...
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12s-P2-DySample.yaml')
    ...
    results = model.train(
        ...
        name='yolo12_P2_DySample_ssdc_uav_exp01',# 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        pretrained='yolo12s.pt',  # 加载官方预训练权重，strict=False
    )
