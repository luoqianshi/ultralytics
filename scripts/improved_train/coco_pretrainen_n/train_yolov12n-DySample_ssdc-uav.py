...
def train():
    """
    YOLOv12-DySample 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv12-DySample 模型，并进行公平性配置。
    """
    # 1. 配置路径
    ...
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12n-DySample.yaml')
    ...
    results = model.train(
        ...
        name='yolo12n_DySample_ssdc_uav_exp01_200Epoch',# 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        pretrained='yolo12n.pt',  # 加载预训练权重
    )
