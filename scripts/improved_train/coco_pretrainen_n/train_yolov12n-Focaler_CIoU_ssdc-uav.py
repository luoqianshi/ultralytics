...
def train():
    ...
    # 2. 加载模型
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12n.yaml')
    ...
    results = model.train(
        ...
        name='yolo12n_Focaler-CIoU_ssdc_uav_exp1',  # 实验名称
        device='0',               # 使用的 GPU 设备索引
        save=True,                # 保存 checkpoint
        optimizer='SGD',          # 使用 SGD 优化器
        pretrained='yolo12n.pt',  # 加载预训练权重，这里使用 COCO 预训练权重
        # Focaler-IoU 配置 (新增)
