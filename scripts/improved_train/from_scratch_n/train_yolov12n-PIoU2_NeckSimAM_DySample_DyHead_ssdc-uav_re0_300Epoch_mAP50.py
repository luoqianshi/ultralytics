...
def train():
    """
`   YOLO12 + DySample + NeckSimAM + DyHead + Powerful-CIoU2 训练脚本
    - 数据集: SSDC-UAV
    ...
    # 2. 加载模型
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\scripts\improved_yolo12\yolo12n-NeckSimAM_DySample_DyHead.yaml')
    ...
    results = model.train(
        ...
        epochs=300,               # 与 baseline 对齐 (E0': 150ep)
        imgsz=640,                # [对齐] 输入图像尺寸
        batch=16,                 # 批次大小 (显存允许的情况下尽量大，16是官方推荐)
        project='runs/ssdc_uav_train',   # 训练结果保存的项目目录
        name='yolo12n_PIoU2_NeckSimAM_DySample_DyHead_ssdc_uav_exp1_re0_300Epoch_mAP50',  # 实验名称
