...
def test_yolov26_ssdc_uav():
    """
    YOLOv26 SSDC-UAV 数据集测试脚本
    
    功能：
    1. 加载训练好的 YOLOv26 模型权重。
    ...
    # 权重文件路径
    weights_path = Path(r'g:\Codes\Python_Codes\Object_Detection_Official_Codabase\ultralytics\scripts\runs\lafssdc_train\yolo12s_lafssdc_exp01\weights\best.pt')
    
    # 数据集配置文件路径
    # 优先使用 cfg/datasets 下的配置文件
    dataset_yaml_path = Path(r'g:\Codes\Python_Codes\Object_Detection_Official_Codabase\ultralytics\ultralytics\cfg\datasets\lafssdc.yaml')
    
    print("=" * 50)
    print("YOLOv26 SSDC-UAV 测试脚本启动")
    print("=" * 50)
    ...
    try:
        ...
        metrics = model.val(
            ...
            name='yolo12s_ssdc_uav_test_exp',
