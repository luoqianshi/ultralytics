...
def test():
    ...
    # 权重文件路径
    # @TODO 记得替换best权重参数
    weights_path = Path(r'D:\Data\New_Codes\Python_Codes\ultralytics\runs\ssdc_uav_train\size-s-base\yolo12s-baseline_ssdc_uav_exp01\weights\best.pt')
    ...
    try:
        ...
        metrics = model.val(
            ...
            name='yolo12s-baseline_ssdc_uav_test_exp01_nms',
            exist_ok=True, # 允许覆盖同名实验目录
            verbose=False,
            iou=0.6,  #  调低推理时的iou阈值，0.7 ——> [0.5,0.6]
            conf=0.001,  #  保持默认的置信度阈值val=0.001, predict=0.25
        )
