...
def train():
    """
    YOLO26 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLO26 模型，并进行公平性配置。
    """
    # 1. 配置路径
    ...
    ## 重零训练模型的配置文件在：ultralytics\cfg\models\26
    ## 注意：yolo26n.yaml 会自动解析为 yolo26.yaml + scale 'n' (约2.57M参数，与其他 n 规格模型对齐)
    model = YOLO(r'ultralytics/cfg/models/26/yolo26n.yaml')
