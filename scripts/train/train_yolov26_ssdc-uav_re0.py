...
def train():
    """
    YOLOv26 训练脚本
    用于在 SSDC-UAV 数据集上训练 YOLOv26 模型，并进行公平性配置。
    """
    # 1. 配置路径
    ...
    # 路径 A: 用户在 prompt 中提到的路径
    yaml_path_a = Path(r'D:\Data\New_Codes\Python_Codes\ultralytics\datasets\SSDC-UAV_yolo\ssdc-uav.yaml')
    
    if yaml_path_a.exists():
    ...
    # 加载配置文件，从零开始训练，而不是在COCO上预训练过的权重参数
    model = YOLO(r'D:\Data\New_Codes\Python_Codes\ultralytics\ultralytics\cfg\models\26\yolo26s.yaml') 
    ...
    results = model.train(
        ...
        name='yolo26s_ssdc_uav_re0_exp01',# 实验名称
